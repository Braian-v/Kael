/**
 * Kael v1.2 — Integration Test Suite
 *
...
async function testReminders() {
  ...
  const expectedISO = new Date("2027-06-15T09:00:00-03:00").toISOString();
...
async function testSpecialEvents() {
  ...
  const { error: ownerErr } = await supabase.from("special_events")
    .insert([{ phone: TEST_PHONE, event_type: "Birthday", person_name: "Braian", event_date: "2026-03-10" }]);
  ownerErr ? log("fail", "Owner birthday stored as Braian", ownerErr.message) : log("pass", "Owner birthday stored as Braian");
}
...
async function testSchedulerLogic() {
  ...
  const todayIST = new Intl.DateTimeFormat("en-CA", { timeZone: "America/Argentina/Cordoba" }).format(new Date());
  const currentHHMM = new Intl.DateTimeFormat("en-GB", { timeZone: "America/Argentina/Cordoba", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date());
...
function testBuildReminderDate() {
  ...
  function buildReminderDate(timeString, dateString = null) {
    const now = new Date();
    if (dateString) return new Date(`${dateString}T${timeString}-03:00`).toISOString();
    const formatter = new Intl.DateTimeFormat("en-US", { timeZone: "America/Argentina/Cordoba", year: "numeric", month: "2-digit", day: "2-digit" });
    const parts = formatter.formatToParts(now);
    const year = parts.find(p => p.type === "year").value;
    const month = parts.find(p => p.type === "month").value;
    const day = parts.find(p => p.type === "day").value;
    const reminderDate = new Date(`${year}-${month}-${day}T${timeString}-03:00`);
    if (reminderDate < now) reminderDate.setDate(reminderDate.getDate() + 1);
    return reminderDate.toISOString();
  }

  buildReminderDate("09:00:00", "2027-06-15") === new Date("2027-06-15T09:00:00-03:00").toISOString()
...
async function main() {
  ...
  console.log(`\x1b[1m\x1b[36m  Kael v1.2 — Integration Test Suite\x1b[0m`);
