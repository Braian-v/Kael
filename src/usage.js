...
async function getUsage() {
  ...
  const ninetyDaysAgo = new Intl.DateTimeFormat("en-CA", { timeZone: "America/Argentina/Cordoba" })
  ...
  for (let i = 89; i >= 0; i--) {
    ...
    const dateStr = new Intl.DateTimeFormat("en-CA", { timeZone: "America/Argentina/Cordoba" }).format(d);
...
async function track(service) {
  ...
  if (error) {
    ...
    const { data } = await supabase.from("api_usage").select(`${service}_count`).eq("usage_date", today).maybeSingle();
