...
const { getHeartbeats, runReminderDispatch, runRoutineDispatch, runRecurringDispatch, runDailySummary } = require("./scheduler");
...
// ---------------------------------------------------------
// REQUIRED SECRETS — fail fast rather than run with webhook
// signature verification silently disabled.
// ---------------------------------------------------------
if (!process.env.WEBHOOK_APP_SECRET) {
  console.error("[security] FATAL: WEBHOOK_APP_SECRET is not set. Refusing to start — webhook signature verification cannot be disabled.");
  process.exit(1);
}
...
// WEBHOOK_APP_SECRET is required (server refuses to start without it — see
// the fail-fast check above), so this always runs.
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// Escapes Postgres ILIKE wildcards (% and _) in user-supplied text so
// characters typed by the user can't be interpreted as pattern wildcards.
// ---------------------------------------------------------
...
// Converts AI-extracted HH:MM:SS to a full Argentina-offset (UTC-3) ISO timestamp
...
// Formats a full ISO timestamp as a local date+time string, e.g. "Jul 14, 9:40 AM"
...
// GET /api/tick with header  x-cron-secret: <CRON_SECRET>
...
// Calling this endpoint runs all four dispatch jobs immediately,
...
//   https://your-app.onrender.com/api/tick
// with a custom header  x-cron-secret: YOUR_CRON_SECRET
// every 1 minute. The secret is NOT accepted as a query param (query
// strings leak into logs, browser history, and referrer headers).
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// STATUS API
// /api/status — feeds the Kael dashboard
// ---------------------------------------------------------
  ...
  try {
    ...
    const jobs = [
      ...
      },
      {
        ...
        schedule: "* * * * *",
        description: "Matches current Argentina local time against active daily routines",
      ...
      },
      {
        name: "Event Alert",
        schedule: "30 8 * * *",
        description: "Double-lock birthday and event alerts at 08:30 local time (Argentina)",
      ...
      },
      {
        name: "Daily Summary",
        schedule: "0 7 * * *",
        description: "Proactively sends a morning digest of today's reminders, routines, events, and pending lists",
        layman: "The Morning Briefing: Texts you first thing, so you never have to ask what's on today.",
        status: "scheduled",
        lastFired: dbJobs?.find(j => j.job_name === 'Daily Summary')?.last_fired || heartbeats['Daily Summary']
      },
    ];
...
app.post("/webhook", async (req, res) => {
  ...
  if (lowerMsg === "hi" || lowerMsg === "hello" || lowerMsg === "hey") {
    const text = isOwner
      ? `Hello Braian. Kael online. You can set reminders, routines, events, search the web, or query your schedule.`
      : `Hello ${senderName}. I am Kael, Braian's personal assistant.`;
    return await replyAndLog(senderPhone, senderName, message, text);
  }

  // 4. CONVERSATIONAL MEMORY — fetch last 8 turns for this sender.
  // Ordered DESC + limit so we actually get the most recent turns (not the
  // oldest — the previous ascending+limit combo always returned the first
  // 4 messages ever sent, never the recent ones), then reversed back to
  // chronological order before handing to the model.
  ...
  if (targetName && targetName.toLowerCase() !== "you") {
    if (!queryOnlyIntents.includes(intent)) {
      const { data: contact } = await supabase
        .from("contacts")
        .select("*")
        .ilike("name", escapeIlike(targetName))
        .maybeSingle();
  ...
  try {
    ...
    if (intent === "web_search") {
      ...
      const summaryPrompt =
        `Kael assistant. Braian asked: "${message}". ` +
    ...
    if (intent === "delete_task") {
      ...
      const [
        ...
        supabase.from("personal_reminders").select("id, message").ilike("message", `%${escapeIlike(cleanTask)}%`).limit(1),
        supabase.from("daily_routines").select("id, task_name").ilike("task_name", `%${escapeIlike(cleanTask)}%`).limit(1),
        supabase.from("recurring_tasks").select("id, task_name").ilike("task_name", `%${escapeIlike(cleanTask)}%`).limit(1),
        supabase.from("special_events").select("id, person_name").ilike("person_name", `%${escapeIlike(cleanTask)}%`).limit(1),
      ]);
    ...
    if (intent === "edit_task") {
      ...
      const { data: matches } = await supabase
        ...
        .eq("status", "pending")
        .ilike("message", `%${escapeIlike(cleanTask)}%`)
    ...
    // Feature: snooze_reminder — postpone a pending reminder, or re-fire a
    // recently completed one after a delay.
    if (intent === "snooze_reminder") {
      if (!isOwner) return await respond("Access denied.");

      const snoozeMinutes = parseInt(aiResult.snoozeMinutes);
      if (!snoozeMinutes || snoozeMinutes < 1) {
        return await respond("Please specify how long to snooze — e.g. \"snooze that 10 minutes\".");
      }

      const cleanTask = (aiResult.editTarget || "").replace(/(routine|reminder|task|event)/gi, "").trim();

      // 1) A pending reminder that hasn't fired yet — just push its time back.
      let pendingQuery = supabase
        .from("personal_reminders")
        .select("*")
        .eq("phone", targetPhone)
        .eq("status", "pending");
      if (cleanTask) pendingQuery = pendingQuery.ilike("message", `%${escapeIlike(cleanTask)}%`);

      const { data: pendingMatches } = await pendingQuery.order("reminder_time", { ascending: true }).limit(1);

      if (pendingMatches?.length > 0) {
        const existing = pendingMatches[0];
        const newTime = new Date(new Date(existing.reminder_time).getTime() + snoozeMinutes * 60_000).toISOString();
        const { error } = await supabase.from("personal_reminders").update({ reminder_time: newTime }).eq("id", existing.id);
        return await respond(
          !error
            ? `Snoozed "${existing.message}" — now at ${formatLocalDateTime(newTime)}.`
            : "Failed to snooze reminder. Please try again."
        );
      }

      // 2) Otherwise, find the most recently fired reminder and re-schedule it.
      let completedQuery = supabase
        .from("personal_reminders")
        .select("*")
        .eq("phone", targetPhone)
        .eq("status", "completed");
      if (cleanTask) completedQuery = completedQuery.ilike("message", `%${escapeIlike(cleanTask)}%`);

      const { data: completedMatches } = await completedQuery.order("reminder_time", { ascending: false }).limit(1);

      if (!completedMatches?.length) {
        return await respond(cleanTask ? `No reminder found matching "${cleanTask}" to snooze.` : "No recent reminder to snooze.");
      }

      const source = completedMatches[0];
      const newTime = new Date(Date.now() + snoozeMinutes * 60_000).toISOString();
      ...
      return await respond(
        !error
          ? `Snoozed "${source.message}" — I'll remind you again at ${formatLocalDateTime(newTime)}.`
          : "Failed to snooze reminder. Please try again."
      );
    }
        ...
        if (oneOff.length > 0) {
          ...
          oneOff.forEach((r) => {
            const t = new Date(r.reminder_time).toLocaleString("en-US", {
              timeZone: "America/Argentina/Cordoba", month: "short", day: "numeric",
    ...
    if (intent === "query_birthday") {
      const { data } = await supabase
        .from("special_events")
        .select("event_date")
        .ilike("person_name", escapeIlike(finalName))
    ...
    if (intent === "event") {
      const eventPersonName = finalName.toLowerCase() === "you" ? "Braian" : finalName;
    ...
    if (intent === "instant_message") {
      const recipientPhone = finalName.toLowerCase() === "you" ? process.env.MY_PHONE_NUMBER : targetPhone;
      try {
        await sendWithTemplateFallback(recipientPhone, `Message from ${senderName}: ${taskOrMessage}`);
      } catch (_) {
        return await respond(
          "Couldn't deliver that message — the recipient hasn't messaged Kael in the last 24 hours and no Message Template is configured for this case. Set WHATSAPP_TEMPLATE_NAME to enable delivery outside that window."
        );
      }
      return await respond(
        finalName.toLowerCase() === "you" ? "Message forwarded." : `Message sent to ${finalName}.`
      );
    }
    ...
    // Feature: free-form notes
    if (intent === "save_note") {
      if (!isOwner) return await respond("Access denied.");
      if (!taskOrMessage || taskOrMessage.trim() === "") return await respond("Please tell me what to note.");

      const { error } = await supabase.from("notes").insert([{
        phone: targetPhone,
        content: taskOrMessage.trim(),
      }]);
      return await respond(!error ? "Note saved." : "Failed to save note. Please try again.");
    }

    if (intent === "query_notes") {
      if (!isOwner) return await respond("Access denied.");

      let query = supabase.from("notes").select("*").eq("phone", targetPhone).order("created_at", { ascending: false });
      if (taskOrMessage && taskOrMessage.trim() !== "") {
        query = query.ilike("content", `%${escapeIlike(taskOrMessage.trim())}%`);
      }
      const { data } = await query.limit(20);

      if (!data || data.length === 0) {
        return await respond(taskOrMessage ? `No notes found matching "${taskOrMessage}".` : "No notes saved yet.");
      }

      let text = "Notes:\n\n";
      data.forEach((n) => (text += `- ${n.content}\n`));
      return await respond(text.trim());
    }

    // Feature: named lists (shopping, todo, etc.)
    if (intent === "list_add") {
      if (!isOwner) return await respond("Access denied.");
      const listName = (aiResult.listName || "").trim().toLowerCase();
      const listItem = (aiResult.listItem || "").trim();

      if (!listName) return await respond("Which list should I add this to?");
      if (!listItem) return await respond("What should I add to the list?");
      ...
      return await respond(
        !error
          ? `Added "${listItem}" to your ${listName} list.`
    ...
    if (intent === "list_view") {
      ...
      if (!listName) {
        // No specific list named — show all lists with pending item counts
        const { data } = await supabase
          .from("lists")
          .select("list_name")
          .eq("phone", targetPhone)
        ...
        if (!data || data.length === 0) return await respond("No lists yet — try \"add milk to the shopping list\".");

        const counts = {};
        data.forEach((row) => { counts[row.list_name] = (counts[row.list_name] || 0) + 1; });

        let text = "Your lists:\n\n";
        Object.entries(counts).forEach(([name, count]) => (text += `- ${name}: ${count} pending\n`));
        return await respond(text.trim());
      }
      ...
      if (!data || data.length === 0) return await respond(`Your ${listName} list is empty.`);

      let text = `${listName.charAt(0).toUpperCase() + listName.slice(1)} list:\n\n`;
      data.forEach((row) => (text += `- ${row.item}\n`));
      return await respond(text.trim());
    }
    ...
    if (intent === "list_remove") {
      ...
      if (!listItem) return await respond("Which item should I remove?");

      let query = supabase
        .from("lists")
        .select("id, item, list_name")
        .eq("phone", targetPhone)
        .eq("is_done", false)
        .ilike("item", `%${escapeIlike(listItem)}%`);
      if (listName) query = query.eq("list_name", listName);

      const { data: matches } = await query.limit(1);
      if (!matches || matches.length === 0) {
        return await respond(`No item matching "${listItem}" found${listName ? ` on your ${listName} list` : ""}.`);
      }

      await supabase.from("lists").delete().eq("id", matches[0].id);
      return await respond(`Removed "${matches[0].item}" from your ${matches[0].list_name} list.`);
    }

    // Feature: finance tracking (expenses/income, optionally tagged by business)
    if (intent === "finance_entry") {
      if (!isOwner) return await respond("Access denied.");

      const entryType = aiResult.entryType === "income" ? "income" : aiResult.entryType === "expense" ? "expense" : null;
      const amount = parseFloat(aiResult.amount);

      if (!entryType) return await respond("Was that money coming in (income) or going out (expense)?");
      if (!amount || amount <= 0) return await respond("Please specify a valid amount.");
      ...
      const { error } = await supabase.from("finance_entries").insert([{
        ...
        business_tag: aiResult.businessTag || null,
        note: taskOrMessage || null,
      }]);

      const tagSuffix = aiResult.businessTag ? ` (${aiResult.businessTag})` : "";
      return await respond(
        !error
          ? `Logged ${entryType} of $${amount}${aiResult.category ? ` — ${aiResult.category}` : ""}${tagSuffix}.`
    ...
    if (intent === "finance_query") {
      ...
      const period = ["today", "week", "month", "all"].includes(aiResult.financePeriod) ? aiResult.financePeriod : "month";
      const now = new Date();
      let since = null;
      if (period === "today") {
        since = new Date(new Date().toLocaleString("en-US", { timeZone: "America/Argentina/Cordoba" }));
      ...
      let query = supabase.from("finance_entries").select("*").eq("phone", targetPhone);
      if (since) query = query.gte("created_at", since.toISOString());
      if (aiResult.businessTag) query = query.ilike("business_tag", escapeIlike(aiResult.businessTag));
      if (taskOrMessage) query = query.ilike("category", `%${escapeIlike(taskOrMessage)}%`);

      const { data } = await query;

      if (!data || data.length === 0) {
        return await respond(`No finance entries found for ${period === "all" ? "any period" : `the last ${period === "today" ? "day" : period}`}${aiResult.businessTag ? ` tagged ${aiResult.businessTag}` : ""}.`);
      }

      const income = data.filter((e) => e.entry_type === "income").reduce((sum, e) => sum + Number(e.amount), 0);
      const expenses = data.filter((e) => e.entry_type === "expense").reduce((sum, e) => sum + Number(e.amount), 0);
      const balance = income - expenses;

      const categoryTotals = {};
      data.filter((e) => e.entry_type === "expense").forEach((e) => {
        const cat = e.category || "uncategorized";
        categoryTotals[cat] = (categoryTotals[cat] || 0) + Number(e.amount);
      });

      const periodLabel = { today: "Today", week: "Last 7 days", month: "Last 30 days", all: "All time" }[period];
      let text = `${periodLabel}${aiResult.businessTag ? ` — ${aiResult.businessTag}` : ""}:\n\n`;
      text += `Income: $${income.toFixed(2)}\n`;
      text += `Expenses: $${expenses.toFixed(2)}\n`;
      ...
      if (catEntries.length > 0) {
        ...
        catEntries
          .sort((a, b) => b[1] - a[1])
          .forEach(([cat, total]) => (text += `- ${cat}: $${total.toFixed(2)}\n`));
      }
...
app.listen(process.env.PORT || 3000, async () => {
  console.log(`[server] Kael v${version} running on port ${process.env.PORT || 3000}`);
